    ...
    <!-- Live as if you were to die tomorrow. Learn as if you were to live forever. - Mahatma Gandhi -->
                                ...
                                @if (count($yates) > 0)
                                    <div class="table-responsive">
                                        <table class="table" id="productos-yates-table">
                                            ...
                                            <tbody>
                                                @forelse($yates as $index => $item)
                                                    <tr>

                                                        <td>
                                                            <div class="productimgname">

                                                                    <div class="view-product bg-success-light">
                                                                        <i class="ti ti-ship text-success"></i>
                                                                    </div>
                                             ...
                                             <tfoot>
                                               <tr class="table-primary fw-bold">
                                                    <td></td>
                                                    <td>{{ number_format($totales['Yate']['horas']) }}</td>
                                                    <td>{{ number_format($totales['Yate']['pax']) }}</td>
                                                    <td class="fw-bold">${{ number_format($totales['Yate']['ventaTotal'], 2) }}</td>
                                                    <td class="text-danger">${{ number_format($totales['Yate']['costo'] , 2) }}</td>
                                                    <td class="text-warning">${{ number_format($totales['Yate']['descuento'], 2) }}</td>
                                                    <td>{{ number_format($totales['Yate']['enm_total'] , 2) }}</td>
                                                    <td class="text-center text-warning">{{ number_format($totales['Yate']['enm_no_pagados_total'] , 2) }}</td>
                                                    <td>{{ number_format($totales['Yate']['fotos_ingresos'] , 2) }}</td>
                                                    <td class="fw-bold text-success">${{ number_format($totales['Yate']['ventaIngresos'] , 2) }}</td>
                                                    <td class="fw-bold text-success">${{ number_format($totales['Yate']['utilidad'] , 2) }}</td>
                                                    <td></td>
                                                </tr>
                                            </tfoot>
                                        </table>

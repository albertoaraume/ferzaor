...
if ($.fn.DataTable.isDataTable('#toursTable')) {
...
$('#tour').off('change.customEvent').on('change.customEvent', function(e) {
clearTimeout(timeout);
timeout = setTimeout(() => {
window.Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id')).set('tourId', $(this).val());
}, 300);
});


$('#proveedor').off('change.customEvent').on('change.customEvent', function(e) {
clearTimeout(timeout);
timeout = setTimeout(() => {
window.Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id')).set('proveedorId', $(this).val());
}, 300);
});

...
<div class="text-center py-5">
    ...
    <h5 class="text-muted mb-2">No hay facturas disponibles</h5>
...
$('#empresa').off('change.customEvent').on('change.customEvent', function(e) {
clearTimeout(timeout);
timeout = setTimeout(() => {
window.Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id')).set('empresaId', $(this).val());
}, 300);
});
...
clearTimeout(timeout);
timeout = setTimeout(() => {
window.Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id')).set('tipoId', $(this).val());
}, 300);
});

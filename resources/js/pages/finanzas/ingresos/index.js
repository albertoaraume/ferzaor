...
window.addEventListener('selectAlll', function() {
...
dataTable = $('#ingresosTable').DataTable({
responsive: false,
autoWidth: false,
scrollX: true,
columnDefs: [
    {
        ...
        /*render: function(data, type, row) {
            return `<input type="checkbox" class="select-row form-check-input" data-id="${row.idIngreso}">`;
        },*/
       // targets: 1
        targets: [0, 1],
...
select: {
    ...
    selector: "td:nth-child(2)" // Segunda columna para selección
},
lengthMenu: [
[10, 25, 50, 100, -1],
[10, 25, 50, 100, "Todos"]
...
<div class="text-center py-5">
    <i class="ti ti-chart-ppf text-muted" style="font-size: 3rem; margin-bottom: 1rem;"></i>
    <h5 class="text-muted mb-2">No hay ingresos disponibles</h5>
...
moverElementosDataTable();
 ...
 // Manejar selección de todos
    $('#select-all').on('change', function() {
        ...
        $('input.select-row', rows).prop('checked', isChecked);
    ...
    $('#ingresosTable tbody').on('change', 'input.select-row', function() {
        var totalCheckboxes = $('#ingresosTable tbody input.select-row:visible').length;
        var checkedCheckboxes = $('#ingresosTable tbody input.select-row:checked').length;
        
        // Actualizar estado del checkbox "select-all"
        var selectAllCheckbox = $('#select-all')[0];
...
function actualizarSeleccionados() {
    var seleccionados = [];
    $('#ingresosTable tbody input.select-row:checked').each(function() {
    ...
    /*if (window.Livewire) {
        window.Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id'))
            .set('ingresosSeleccionados', seleccionados);
    }*/
}

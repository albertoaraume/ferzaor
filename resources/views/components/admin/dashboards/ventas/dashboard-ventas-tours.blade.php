    ...
    <!-- Live as if you were to die tomorrow. Learn as if you were to live forever. - Mahatma Gandhi -->
                                ...
                                @if (count($tours) > 0)
                                    <div class="table-responsive">
                                        <table class="table" id="productos-tours-table">
                                            ...
                                            <tbody>
                                             ...
                                             <tfoot>
                                               <tr class="fw-bold" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                                                    <td></td>
                                                    <td>{{ number_format($totales['Tour']['horas']) }}</td>
                                                    <td>{{ number_format($totales['Tour']['pax']) }}</td>
                                                    <td class="fw-bold">${{ number_format($totales['Tour']['ventaTotal'], 2) }}</td>
                                                    <td class="text-danger">${{ number_format($totales['Tour']['costo'] , 2) }}</td>
                                                    <td class="text-warning">${{ number_format($totales['Tour']['descuento'], 2) }}</td>
                                                    <td>{{ number_format($totales['Tour']['enm_total'] , 2) }}</td>
                                                    <td class="text-center text-warning">{{ number_format($totales['Tour']['enm_no_pagados_total'] , 2) }}</td>
                                                    <td>{{ number_format($totales['Tour']['fotos_ingresos'] , 2) }}</td>
                                                    <td class="fw-bold text-success">${{ number_format($totales['Tour']['ventaIngresos'] , 2) }}</td>
                                                    <td class="fw-bold text-success">${{ number_format($totales['Tour']['utilidad'] , 2) }}</td>
                                                    <td></td>
                                                </tr>
                                            </tfoot>
                                        </table>

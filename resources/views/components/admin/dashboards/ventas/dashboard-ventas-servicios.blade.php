    ...
    <!-- Live as if you were to die tomorrow. Learn as if you were to live forever. - Mahatma Gandhi -->
                                ...
                                @if (count($servicios) > 0)
                                    <div class="table-responsive">
                                        <table class="table" id="productos-servicios-table">
                                            ...
                                            <tbody>
                                             ...
                                             <tr class="table-primary fw-bold">
                                                        <td></td>
                                                    <td>{{ number_format($totales['Servicio']['horas']) }}</td>
                                                    <td>{{ number_format($totales['Servicio']['pax']) }}</td>
                                                    <td class="fw-bold">${{ number_format($totales['Servicio']['ventaTotal'], 2) }}</td>
                                                    <td class="text-danger">${{ number_format($totales['Servicio']['costo'] , 2) }}</td>
                                                    <td class="text-warning">${{ number_format($totales['Servicio']['descuento'], 2) }}</td>
                                                    <td>{{ number_format($totales['Servicio']['enm_total'] , 2) }}</td>
                                                    <td class="text-center text-warning">{{ number_format($totales['Servicio']['enm_no_pagados_total'] , 2) }}</td>
                                                    <td>{{ number_format($totales['Servicio']['fotos_ingresos'] , 2) }}</td>
                                                    <td class="fw-bold text-success">${{ number_format($totales['Servicio']['ventaIngresos'] , 2) }}</td>
                                                    <td class="fw-bold text-success">${{ number_format($totales['Servicio']['utilidad'] , 2) }}</td>
                                                    <td></td>
                                                </tr>
                                            </tfoot>
                                        </table>

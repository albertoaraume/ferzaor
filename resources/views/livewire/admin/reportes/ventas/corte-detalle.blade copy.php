<div>
    ...
    @if ($corte->ventas->count() > 0)
        <div class="card mb-4">
            <div class="card-header">
                <h5 class="card-title mb-0"><i class="ti ti-shopping-cart me-2"></i>Registro de Ventas
  ...
  @include('livewire.admin.reportes.ventas.partials.corte.fotos', ['corte' => $corte])
    <!-- Listado de Reservas -->
    @if ($corte->reservas->count() > 0)
        <div class="card mb-4">
            <div class="card-header">
                <h5 class="card-title mb-0"><i class="ti ti-calendar-event me-2"></i>Reservas Relacionadas
                    ({{ $corte->reservas->count() }})</h5>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @foreach ($corte->reservas as $reserva)
                                <tr class="{{ $reserva->edo == false ? 'table-danger' : '' }}"
                                    style="{{ $reserva->edo == false ? 'text-decoration: line-through; text-decoration-color: #dc3545; text-decoration-thickness: 2px;' : '' }}">

                                    <td>{{ $reserva->idReserva }}</td>
                                    <td>{{ $reserva->reserva->folio }}</td>
                                    <td>{{ $reserva->reserva->nombreCliente ?? 'N/A' }}</td>
                                    <td>{{ $reserva->venta->c_moneda }}</td>
                                    <td
                                        class="{{ $reserva->venta->c_moneda == 'USD' && $reserva->precio > 0 ? 'text-info' : 'text-muted' }}">
                                        <strong>${{ number_format($reserva->venta->c_moneda == 'USD' ? $reserva->precio : 0, 2) }}</strong>
                                    </td>
                                    <td
                                        class="{{ $reserva->venta->c_moneda == 'MXN' && $reserva->precio > 0 ? 'text-info' : 'text-muted' }}">
                                        <strong>${{ number_format($reserva->venta->c_moneda == 'MXN' ? $reserva->precio : 0, 2) }}</strong>
                                    </td>
                                    <td
                                        class="{{ $reserva->venta->c_moneda == 'USD' && $reserva->descuento > 0 ? 'text-info' : 'text-muted' }}">
                                        <strong>${{ number_format($reserva->venta->c_moneda == 'USD' ? $reserva->descuento : 0, 2) }}</strong>
                                    </td>
                                    <td
                                        class="{{ $reserva->venta->c_moneda == 'MXN' && $reserva->descuento > 0 ? 'text-info' : 'text-muted' }}">
                                        <strong>${{ number_format($reserva->venta->c_moneda == 'MXN' ? $reserva->descuento : 0, 2) }}</strong>
                                    </td>
                                    <td
                                        class="{{ $reserva->venta->c_moneda == 'USD' && $reserva->importe > 0 ? 'text-info' : 'text-muted' }}">
                                        <strong>${{ number_format($reserva->venta->c_moneda == 'USD' ? $reserva->importe : 0, 2) }}</strong>
                                    </td>
                                    <td
                                        class="{{ $reserva->venta->c_moneda == 'MXN' && $reserva->importe > 0 ? 'text-info' : 'text-muted' }}">
                                        <strong>${{ number_format($reserva->venta->c_moneda == 'MXN' ? $reserva->importe : 0, 2) }}</strong>
                                    </td>
                                </tr>
                            @endforeach
